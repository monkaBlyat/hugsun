	...
	virtMemTable[i] = kmalloc(K_BLOCK_SIZE+2*PAGE_SIZE,GFP_KERNEL);
		...
		memMgrFreeMem();
      	...
      	else
      	{
        	...
        	kmalloc_area = (unsigned long *)(((unsigned long)virtMemTable[i] + PAGE_SIZE -1) & PAGE_MASK);

          	physMemTable_area[i] = virt_to_phys(virtMemTable[i]);
                printk("physMemTable_area[%d] =  %p \n",i,physMemTable_area[i] );
...
int memMgrdrv_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    {
//    	printk("memMgrdrv_mmap : size too big %ld\n",size);
	return(-ENXIO);
    }
    ...
    {       
        ...
        virt_to_phys((void*)((uint64_t)physMemTable_area))>> PAGE_SHIFT,
