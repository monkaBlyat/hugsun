...
extern int LT9211_init_module(void);
extern void LT9211_exit_module(void);
		...
		for (retries = 0; retries < 100; retries++) {
			if (dw_pcie_link_up(pci)) {
				...
				if (dw_pcie_link_up(pci)) {
					...
					node = pci->dev->of_node;
					if (node) {
						const char *node_name = of_node_full_name(node);
						printk("@pan 2024 Node full name: %s\n", node_name);
						if (strstr(node_name, "pcie@fe170000")) {
							printk("@pan add tl9211 mipi to lvds\n");
							LT9211_init_module();
						}
					} else {
						printk("@pan Node is NULL\n");
					}
