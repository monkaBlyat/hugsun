...
//			mac_addr_efuse[0], mac_addr_efuse[1], mac_addr_efuse[2],
//			mac_addr_efuse[3], mac_addr_efuse[4], mac_addr_efuse[5]);
	if(mac_addr_efuse[0] == 0x11){       //fix 0x11 is invalid MAC
		memcpy(init_conf.mac_addr, dflt_mac, ETH_ALEN);
//		printk("11==mac_addr_efuse[0]=%02x\n", mac_addr_efuse[0]);
	}else if (mac_addr_efuse[0] | mac_addr_efuse[1] | mac_addr_efuse[2] | mac_addr_efuse[3]) {
		memcpy(init_conf.mac_addr, mac_addr_efuse, ETH_ALEN);
//		printk("12==mac_addr_efuse[0]=%02x\n", mac_addr_efuse[0]);
	}
//	printk("==========get macaddr: %02x:%02x:%02x:%02x:%02x:%02x\r\n",
	AICWFDBG(LOGINFO, "get macaddr: %02x:%02x:%02x:%02x:%02x:%02x\r\n",
			init_conf.mac_addr[0], init_conf.mac_addr[1], init_conf.mac_addr[2],
			init_conf.mac_addr[3], init_conf.mac_addr[4], init_conf.mac_addr[5]);
