...
static int es8323_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	gpiod_set_value_cansleep(es8323->amp_en_gpio, !mute);
...
static int es8323_probe(struct snd_soc_component *component)
{
	...
	es8323->amp_en_gpio = devm_gpiod_get_optional(component->dev,"amp-en",GPIOD_OUT_LOW);
	if (IS_ERR(es8323->amp_en_gpio))
			return PTR_ERR(es8323->amp_en_gpio);

	gpiod_set_value_cansleep(es8323->amp_en_gpio, 0);
